...
class AddTempPermission : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        ...
        if (args.size != 3) {
            player.sendMessage("${de.marylieh.simpleperms.Main.error}Please use: §9/sptemp <user> <permission> <time>")
            return true
        }
        val userName = args[0]
        ...
        val time: Long
        if (user == null) {
            player.sendMessage("${de.marylieh.simpleperms.Main.error}The player is not Online.")
            return true
        }
        if (user.hasPermission(perm)) {
            player.sendMessage("${de.marylieh.simpleperms.Main.error}The Player has already the permission: §4" + perm)
            return true
        }
        try {
            time = args[2].toLong()
            ...
            e.printStackTrace()
            player.sendMessage("${de.marylieh.simpleperms.Main.error}Please use a NUMBER for the TIME")
        }
        return true
    }
}